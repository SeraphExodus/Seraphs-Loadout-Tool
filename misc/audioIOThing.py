...
def animate(i):
	...
	yf = np.abs(fft(data_np))
	# The fft will return complex numbers, so np.abs will return their magnitude

	max = np.max(yf[0:CHUNK])
	xcoord = list(yf[0:CHUNK]).index(max) * RATE/CHUNK

	line_fft.set_xdata(np.arange(0, RATE, RATE/CHUNK))
	line_fft.set_ydata(yf[0:CHUNK]  / (512 * CHUNK))
	maxdot.set_xdata([xcoord])
	maxdot.set_ydata([max / (512 * CHUNK)])
	print(int(np.floor(xcoord)))
