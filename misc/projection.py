...
for observer in spherePoints:
    ...
    precision = max([max(x) for x in boxes])/100

    lowestX = precisionRounding(min([x[0] for x in vertexProjection]),precision,'floor')
    highestX = precisionRounding(max([x[0] for x in vertexProjection]),precision,'ceil')
    lowestY = precisionRounding(min([x[1] for x in vertexProjection]),precision,'floor')
    highestY = precisionRounding(max([x[1] for x in vertexProjection]),precision,'ceil')

    #randPoints = int((max(highestX,highestY)-min(lowestX,lowestY))/precision)

    #xRange = np.random.rand(randPoints)*(highestX-lowestX) + lowestX
    #yRange = np.random.rand(randPoints)*(highestY-lowestY) + lowestY

    #xRange = np.arange(lowestX,highestX+precision,precision)
    #yRange = np.arange(lowestY,highestY+precision,precision)

    simMax = int(max(highestX-lowestX,highestY-lowestY)/precision)**2
    simArea = (highestX-lowestX) * (highestY-lowestY)
    ...
    for n in range(0,simMax):
        x = np.random.rand(1)[0]*(highestX-lowestX) + lowestX
        y = np.random.rand(1)[0]*(highestY-lowestY) + lowestY
        xRange.append(x)
        yRange.append(y)
        check1List = []
        check2List = []
        for edgePair in edgePairs:
            for face in edgePair: #check every face to see if it intersects any of them
                check1 = False
                check2 = False
    ...
    area = len(pointCount[0]) / len(xRange) * simArea
    areaList.append(area)
    ax.scatter(xRange,yRange,color='#ff0000',s=1)
    ax.scatter(pointCount[0],pointCount[1],color='#333333',s=1)
