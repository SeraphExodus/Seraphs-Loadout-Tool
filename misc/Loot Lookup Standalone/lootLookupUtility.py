    ...
    if len(selection) != 0:
        ...
        for i in brandOdds:
            try:
                ...
                selectionTable.append([i[0], 'Improbable', str(round(selectionDensity[brandOdds.index(i)]*100,2)) + '%', 'Improbable'])
...
def generateDropRateChart(lootLookupWindow):
    ...
    if selection in shipStrings:
        ...
        for i in range(0,len(means)):
            stdev = means[i] * mods[i] / 2
            zScore = (value - means[i])/stdev
            rarity.append(normalCDF(zScore))
        if tail == 1:
            rarity = [1-x for x in rarity]
        ...
        try:
            ...
            if count not in ['','0']:
                ...
                if "kash nunes".casefold() in selection.casefold():
                    tokenMult = int(level[1]) * 5 + 50
                    count = int(count/tokenMult)
                    expected = int(count*p)
                    if expected > 25:
                        count = int(25/p*tokenMult)
                        lootLookupWindow['tokenskills'].update(count)
                        reset = True
                else:
                    count = int(count)
                    expected = int(count*p)
                ...
                try:
                    ...
                    apex = 0
                low = int(max(0,expected-20))
                high = int(expected+20)
                x2 = range(low,high+1)
                for k in x2:
                    try:
...
def lootLookup():
    ...
    # sourceListFrame = [
    #     [sg.VPush()],
    #     [sg.Push(),sg.Text("Level",font=baseFont,p=1,s=10,justification='center'),sg.Push(),sg.Text("Component Name",font=baseFont,p=1,s=40,justification='center'),sg.Push(),sg.Text("Drop Rate",font=baseFont,p=1,s=10,justification='center'),sg.Push()],
    #     [sg.VPush()]
    # ]

    # for i in range(0,20):
    #     newLine = [sg.Push(),sg.Text("SampleLevel",font=baseFont,p=1,key='source' + str(i) + 'level',s=10,justification='center'),sg.Push(),sg.Text("SampleText",font=baseFont,p=1,key='source' + str(i) + 'comp',s=40,justification='center'),sg.Push(),sg.Text("SampleOdds",font=baseFont,p=1,key='source' + str(i) + 'rate',s=10,justification='center'),sg.Push()]
    #     sourceListFrame.append(newLine)

    # sourceListFrame.append([sg.VPush()])

    Layout = [
        [
            sg.Frame('',selectFrame,border_width=0),
            # sg.Frame('',sourceListFrame,border_width=0),
            ...
            sg.Table(values=[],headings=['Loot Source','Odds'],col_widths=[40, 30], key='sourcestable',num_rows=25, auto_size_columns=False,enable_click_events=True, enable_events=True, cols_justification=['center','center'], header_font=baseFont, font=('Roboto',10),visible=False),
    ...
    lootLookupWindow = sg.Window("Loot Lookup Utility",Layout,modal=True,icon=os.path.abspath(os.path.join(os.path.dirname(__file__), 'SLT_Icon.ico')),finalize=True)
    ...
    while True:
        ...
        if event == 'modeselect':
            ...
            if newMode == 'View Loot Tables':
            ...
            elif newMode == 'Find Best Sources':
                try:
                ...
                if values['componentselect'] not in ['Any',''] and values['relevelselect'] not in ['Any','']:
                    ...
                    lootLookupWindow['posttext'].update('',visible=False)
                    lootLookupWindow['postvalue'].update('',visible=False)
            elif newMode == 'View Drop Rate Charts':
                lootLookupWindow['shipsdropdowntext'].update('Loot Source:', visible=True)
        ...
        try:
            if event.endswith("+FOCUS OUT") and '' not in [values['componentselect'],values['relevelselect']] and tryFloat(values['inputvalue']) != 0 and values['inputvalue'] != '' and values['modeselect'] == 'View Drop Rate Charts':
                shipsTable = calculateBestSources(lootLookupWindow)
                entry = values['shipsdropdown']
                if entry != '' and any([entry.casefold() in x[0].casefold() for x in shipsTable]):
                    ...
                    filteredValues.sort()
                    lootLookupWindow['shipsdropdown'].update(value=entry,values=filteredValues)
                x, y, xalt, yalt, reset = generateDropRateChart(lootLookupWindow)
                if reset:
                    x, y, xalt, yalt, reset = generateDropRateChart(lootLookupWindow)
                ax.cla()
                ax2.cla()
        ...
        if event == 'sourcestable':
            try:
                if values['inputstat'] != '' and values['inputvalue'] != 0:
                    ...
                    selectionTable = [[selection, '', '', selectedRow[1]],['','','','']] + selectionTable
                    lootLookupWindow['brandtable'].update(values=selectionTable, visible=True)
            except:
                pass
