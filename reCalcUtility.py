...
def getMatches(reCalcWindow):
    ...
    if blankStats + rewardStats == len(compStats):
        return [''] * 9, rarityList1inx, '', [''] * 9, [''] * 9, [''] * 9, [''] * 9
    ...
    return rarityList, rarityList1inx, rarity, matches, matchesRaw, postRE, logDelta
...
def updateMatchQuality(logDeltas, reCalcWindow):
    ...
    for i in range(0,len(logDeltas)):
        ...
        elif logDeltas[i] != '' and len(blanks) != len(logDeltas) - 1:
            if compStats[i] == target:
                reCalcWindow['matchquality' + str(i)].update('Match Target', text_color='#ff42ff')
            elif abs(logDeltas[i]) < threshold:
                reCalcWindow['matchquality' + str(i)].update('Great Match', text_color='#00ffff')
            elif abs(logDeltas[i]) < 2 * threshold:
                reCalcWindow['matchquality' + str(i)].update('Good Match', text_color='#00ee00')
            elif abs(logDeltas[i]) < 3 * threshold:
                reCalcWindow['matchquality' + str(i)].update('Passable Match', text_color='#aaff44')
            elif abs(logDeltas[i]) < 4 * threshold:
                reCalcWindow['matchquality' + str(i)].update('Slight Mismatch', text_color='#ffee00')
            elif abs(logDeltas[i]) < 5 * threshold:
                reCalcWindow['matchquality' + str(i)].update('Moderate Mismatch', text_color='#ffaa00')
            elif abs(logDeltas[i]) < 6 * threshold:
                reCalcWindow['matchquality' + str(i)].update('Significant Mismatch', text_color='#ff5500')
            elif abs(logDeltas[i]) >= 6 * threshold:
                reCalcWindow['matchquality' + str(i)].update('Critical Mismatch!', text_color='#ff0000')
            else:
                reCalcWindow['matchquality' + str(i)].update('', text_color=textColor)
        else:
            reCalcWindow['matchquality' + str(i)].update('', text_color=textColor)

    if compType != 'Armor' and compStats[0] != target and values['statinput0'] != '':
        reCalcWindow['matchquality0'].update('Not Considered', text_color=textColor)
    
    reCalcWindow.refresh()

def brandTable(reCalcWindow):
    ...
    rarityList, rarityList1inx, rarityout, matches, matchesRaw, postRE, logDelta = getMatches(reCalcWindow)
    ...
    for i in range(0,len(stats)):
        ...
        if values['statinput' + str(i)] != '':
            newCol.append([sg.Push(),sg.Text(rawTableStats[i],font=baseFont,p=0),sg.Push()])
            newCol.append([sg.Push(),sg.Text(reCalcWindow['statoutput' + str(i)].get(),font=baseFont,p=0),sg.Push()])
            newCol.append([sg.Push(),sg.Text(rarityList1inx[i],font=baseFont,p=0),sg.Push()])
            newCol.append([sg.Push(),sg.Text(logDelta[i],font=baseFont,p=0),sg.Push()])
        else:
            newCol.append([sg.Push(),sg.Text(reCalcWindow['matchoutput' + str(i)].get(),font=('Roboto',10,'italic'),p=0),sg.Push()])
            newCol.append([sg.Push(),sg.Text(reCalcWindow['matchpost' + str(i)].get(),font=('Roboto',10,'italic'),p=0),sg.Push()])
            newCol.append([sg.Push(),sg.Text(targetRarity,font=('Roboto',10,'italic'),p=0),sg.Push()])
            newCol.append([sg.Push(),sg.Text('-',font=('Roboto',10,'italic'),p=0),sg.Push()])
        newCol.append([sg.Text('',font=summaryFont,p=0)])
        newCol.append([sg.Push(),sg.Text(stats[i],font=summaryFont,p=0),sg.Push()])
        for j in range(0,len(brandNames)):
            newCol.append([sg.Push(),sg.Text(rarityTable[i][j],font=baseFont,p=0,text_color=rarityColors[i][j]),sg.Push()])
        Layout[0].append(sg.Frame('',newCol,border_width=0,p=elementPadding))

    brandWindow = sg.Window("Brand Rarity Breakdown",Layout,modal=True,icon=os.path.abspath(os.path.join(os.path.dirname(__file__), 'SLT_Icon.ico')),finalize=True)

    brandWindow.bind('<Control-c>','Capture Screenshot')

    while True:
        event2, values2 = brandWindow.read()

        if event2 == 'Capture Screenshot':
...
def reAnalysisUI(reCalcWindow):
    ...
    for i in range(0,len(compStats)):
        statsColumn.append([sg.Push(),sg.Text(compStats[i],font=baseFont,p=0)])
        if values['statinput' + str(i)] != '':
            if compStats[i] in ['Vs. Shields:','Vs. Armor:','Refire Rate:']:
                worstCase = "{:.3f}".format(postsList[i][0])
                worstCasePercent = ' (' + str(round(percentsList[i][0],1)) + '%)'
                bestCase = "{:.3f}".format(postsList[i][1])
                bestCasePercent = ' (' + str(round(percentsList[i][1],1)) + '%)'
                worstCasePre = "{:.3f}".format(postsList[i][2])
                worstCasePrePercent = ' (' + str(round(percentsList[i][2],1)) + '%)'
                bestCasePre = "{:.3f}".format(postsList[i][3])
                bestCasePrePercent = ' (' + str(round(percentsList[i][3],1)) + '%)'
                visibility = True
            elif compStats[i] == 'Recharge:' and compType == 'Shield':
                worstCase = "{:.2f}".format(postsList[i][0])
                worstCasePercent = ' (' + str(round(percentsList[i][0],1)) + '%)'
                bestCase = "{:.2f}".format(postsList[i][1])
                bestCasePercent = ' (' + str(round(percentsList[i][1],1)) + '%)'
                worstCasePre = ''
                worstCasePrePercent = ''
                bestCasePre = ''
                ...
                worstCase = "{:.1f}".format(postsList[i][0])
                worstCasePercent = ' (' + str(round(percentsList[i][0],1)) + '%)'
                bestCase = "{:.1f}".format(postsList[i][1])
                bestCasePercent = ' (' + str(round(percentsList[i][1],1)) + '%)'
                worstCasePre = ''
                worstCasePrePercent = ''
                bestCasePre = ''
                bestCasePrePercent = ''
            worstCaseStatColumn.append([sg.Push(),sg.Text(worstCase,font=baseFont,p=0)])
            worstCasePercentColumn.append([sg.Text(worstCasePercent,font=baseFont,p=0),sg.Push()])
            bestCaseStatColumn.append([sg.Push(),sg.Text(bestCase,font=baseFont,p=0)])
            bestCasePercentColumn.append([sg.Text(bestCasePercent,font=baseFont,p=0),sg.Push()])
            worstCasePreStatColumn.append([sg.Push(),sg.Text(worstCasePre,font=baseFont,p=0)])
            worstCasePrePercentColumn.append([sg.Text(worstCasePrePercent,font=baseFont,p=0),sg.Push()])
            bestCasePreStatColumn.append([sg.Push(),sg.Text(bestCasePre,font=baseFont,p=0)])
            bestCasePrePercentColumn.append([sg.Text(bestCasePrePercent,font=baseFont,p=0),sg.Push()])
            roundingColumn.append([sg.Push(),sg.Text(rounding[i],font=baseFont,p=0),sg.Push()])
            if reCalcWindow['statsheader'].get() == 'Input Raw Component Stats':
                rawStatsColumn.append([sg.Push(),sg.Text(values['statinput' + str(i)],font=baseFont,p=0),sg.Push()])
            else:
                rawStatsColumn.append([sg.Push(),sg.Text(reCalcWindow['statoutput' + str(i)].get(),font=baseFont,p=0),sg.Push()])
        else:
            worstCaseStatColumn.append([sg.Push(),sg.Text('',font=baseFont,p=0)])
            worstCasePercentColumn.append([sg.Text('',font=baseFont,p=0),sg.Push()])
            bestCaseStatColumn.append([sg.Push(),sg.Text('',font=baseFont,p=0)])
            bestCasePercentColumn.append([sg.Text('',font=baseFont,p=0),sg.Push()])
            worstCasePreStatColumn.append([sg.Push(),sg.Text('',font=baseFont,p=0)])
            worstCasePrePercentColumn.append([sg.Text('',font=baseFont,p=0),sg.Push()])
            bestCasePreStatColumn.append([sg.Push(),sg.Text('',font=baseFont,p=0)])
            bestCasePrePercentColumn.append([sg.Text('',font=baseFont,p=0),sg.Push()])
            roundingColumn.append([sg.Push(),sg.Text('',font=baseFont,p=0),sg.Push()])
            rawStatsColumn.append([sg.Push(),sg.Text('',font=baseFont,p=0),sg.Push()])
    ...
    analysisWindow = sg.Window("Reverse Engineering Analysis",Layout,modal=True,icon=os.path.abspath(os.path.join(os.path.dirname(__file__), 'SLT_Icon.ico')),finalize=True)
...
def reCalc():
    ...
    for i in range(0,9):
        ...
        matchesOutputs.append([sg.Text("",s=8,key='matchoutput' + str(i),p=(0,2),font=baseFont)])
        matchesPost.append([sg.Text("",s=8,key='matchpost' + str(i),p=(0,2),font=baseFont)])
    ...
    matchingConfigFrameRight = [
        [sg.Combo(values=['Average Rarity','Best Stat','Worst Stat'],default_value="Average Rarity",key='matchingtarget',enable_events=True,readonly=True,size=(17,12),font=baseFont,p=1),sg.Push()],
    ...
    matchesFrame = [
        ...
        [sg.Push(),sg.vtop(sg.Frame('',matchesText,border_width=0,p=elementPadding)),sg.vtop(sg.Frame('',matchesOutputs,border_width=0,p=elementPadding,s=(65,210))),sg.vtop(sg.Frame('',matchesPost,border_width=0,p=elementPadding,s=(65,210))),sg.Push()]
    ]

    IOFrame = [
        [sg.Push(),sg.Frame('',selectFrame,border_width=0,p=elementPadding,s=(325,85),element_justification='center'),sg.Push()],
        [sg.VPush()],
    ...
    matchingFrame = [
        [sg.Push(),sg.Frame('',matchingConfigFrame,border_width=0,p=elementPadding,s=(265,85)),sg.Push()],
        [sg.VPush()],
        [sg.Push(),sg.Frame('',matchesFrame,border_width=0,p=0,s=(265,250),element_justification='center'),sg.Push()]
    ]  
    ...
    reCalcWindow = sg.Window("Reverse Engineering Calculator",Layout,modal=True,icon=os.path.abspath(os.path.join(os.path.dirname(__file__), 'SLT_Icon.ico')),size=(1120,350),finalize=True)
    ...
    while True:
        ...
        if event == 'Brand Rarity Table':
            brandTable(reCalcWindow)
            remodalize(reCalcWindow) 
        
        if event == 'RE Analysis':
            reAnalysisUI(reCalcWindow)
            remodalize(reCalcWindow)
        ...
        if event == 'New Project':
            ...
            if result == 'Proceed':
                ...
                for i in fields:
                    for j in range(0,9):
                        if i == 'statinput':
                            reCalcWindow[i + str(j)].update('',disabled=True,visible=False)
                        else:
                            reCalcWindow[i + str(j)].update('')
                
                reCalcWindow['matchingtarget'].update('Average Rarity')
                reCalcWindow['targetrarity'].update('')
        ...
        if event == 'Open Project':
            ...
            if loaded:
                ...
                rarityList, rarityList1inx, rarity, matches, matchesRaw, postRE, logDelta = getMatches(reCalcWindow)
                updateMatchQuality(logDelta,reCalcWindow)
                logDelta = formatLogDelta(logDelta)
                if rarity not in [0, '']:
        ...
        try:
            if event.endswith("+FOCUS OUT"):
                ...
                updateREOutputs(reCalcWindow)
                rarityList, rarityList1inx, rarity, matches, matchesRaw, postRE, logDelta = getMatches(reCalcWindow)
                isUnicorn(rarityList,reCalcWindow)
                updateMatchQuality(logDelta,reCalcWindow)
                logDelta = formatLogDelta(logDelta)
                if rarity not in [0, '']:
                    reCalcWindow['targetrarity'].update('1 in ' + str(int(1/tryFloat(rarity))))
