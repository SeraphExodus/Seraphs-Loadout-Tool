...
def getShipInfo(selection):
    ...
    for i in range(0,8):
        if i%2 == 0:
            ...
            right.append([sg.VPush()])
            
            leftWeaponFrames.append(left)
            rightWeaponFrames.append(right)
        
    dataFrames = [sg.Frame('',dataFrame1,border_width=0,p=0),sg.Frame('',dataFrame2,border_width=0,p=0)]

    for i in range(0,4):
        dataFrames.append(sg.Frame('',leftWeaponFrames[i],border_width=0,p=0))
        dataFrames.append(sg.Frame('',rightWeaponFrames[i],border_width=0,p=0))

    dataFrames = [dataFrames]

    Layout = [
        [sg.Push(),sg.Text(selection,font=baseFont,p=0),sg.Push()],
        [sg.Push(),sg.Text(shipType,font=baseFont,p=0),sg.Push()],
...
def lootLookup():
    ...
    lootGroupsFrame.append([sg.Push(),sg.Button('Ship Info',font=buttonFont),sg.Push()])
    ...
    while True:
        event, values = lootLookupWindow.read(timeout=150)

        if event == 'Ship Info':
            try:
                getShipInfo(values['shipsdropdown'])
            except:
                pass
        ...
        if event == 'modeselect':
            ...
            if newMode == 'View Loot Tables' and oldMode != newMode:
                ...
                lootLookupWindow['lootgrouptext'].update('')
                lootLookupWindow['lootgroup'].update('')
                lootLookupWindow['dropcounttext1'].update('')
                lootLookupWindow['dropcounttext2'].update('')
                lootLookupWindow['dropcounttext3'].update('')
                lootLookupWindow['dropcountvalue1'].update('')
                lootLookupWindow['dropcountvalue2'].update('')
                lootLookupWindow['dropcountvalue3'].update('')
                for i in range(1,7):
                    lootLookupWindow['loottable' + str(i)].update('')
