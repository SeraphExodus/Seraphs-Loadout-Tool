...
def getMatches(reCalcWindow):
    ...
    for i in range(0,len(compStats)):
        ...
        #attempt to account for matching disparity when parts can roll negative mass. This only is an issue for armor so for now I'm only gonna implement it as such TO-DO change this later to include all. It doesn't cost much comp time.
        if compType == 'Armor' and compStats[i] == 'Mass':
            zeroMass = getRarity(0,means[i],stdevs[i],mixtureWeights)
            rarity = tryFloat(getRarity(stats[i],means[i],stdevs[i],mixtureWeights)) - zeroMass
            if rarity < 0:
                rarity = 0
        else:
    ...
    averageClamp = unicornThreshold / pow(10,1/3)
    statClamp = unicornThreshold / pow(10,1/2)
...
def updateConfigPane(rarity, threshold, reCalcWindow):
    if rarity not in [0, ''] and formatRarity(1/(tryFloat(rarity))) != 'Improbable':
        if reCalcWindow['matchingtarget'] == 'Average Rarity':
            ceiling = threshold / pow(10,1/3)
        else:
            ceiling = threshold / pow(10,1/2)
...
def reCalc():
    ...
    reCalcWindow = sg.Window("Reverse Engineering Calculator",Layout,modal=False,icon=os.path.abspath(os.path.join(os.path.dirname(__file__), 'SLT_Icon.ico')),size=(1120,365),finalize=True)
    ...
    while True:
        thisWindow, event, values = sg.read_all_windows()

        if thisWindow == reCalcWindow:
            ...
            if event == 'Open Project':
                ...
                if loaded:
                    ...
                    logDelta = formatLogDelta(logDelta)
                    unicorns, unicornThreshold = isUnicorn(rarityList, reCalcWindow)
                    updateConfigPane(rarity,unicornThreshold,reCalcWindow)
                    for i in range(0,9):
                        try:
            ...
            if thisWindow == reCalcWindow and (event == sg.WIN_CLOSED or event == 'Exit'):
                reCalcWindow.close()
                db.close()
                return export
